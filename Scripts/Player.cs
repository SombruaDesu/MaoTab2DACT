...
public partial class Player : CharacterBody2D
{
    ...
    private AnimatedSprite2D _sprite; // 精灵渲染对象
    private Ray      _ray;    // 射线对象
    
    // 当前速度（使用 CharacterBody2D 提供的 Velocity）
    private Vector2 _targetVelocity = Vector2.Zero;

    // 外力相关：瞬时外力（一次性施加，例如后坐力），以及持续外力（例如风场）
    private Vector2 _externalImpulse = Vector2.Zero; // 每帧施加一次后重置
    ...
    private void Flip()
    {
        ...
        {
            _sprite.FlipH = true;
            _ray.SetTargetPosition(new Vector2(-50, 0));
        }
        else
        {
            _sprite.FlipH = false;
    ...
    private void UpdateVertical(float dt)
    {
        ...
        {
            // 根据角色重量调节跳跃冲量
            float effectiveImpulse = Data.JumpImpulse * (100f / Weight);
            bool  hasJumped        = false;
            // 记录跳跃冲力（绝对值）
            _currentJumpImpulse = effectiveImpulse;
            
            // 如果在地面或者在狼跳时间内，则启动第一次跳跃
            if (IsOnFloor() || _coyoteTimer > 0)
            {
                _targetVelocity.Y = -effectiveImpulse;
                _jumpCount        = 1;
            ...
            {
                float horizontalBoost = effectiveImpulse * JumpDirectionalBoostFactor;
    ...
    /// <param name="isRun">isRun 为 true 时表示奔跑（但本逻辑中按下时为步行）</param>
